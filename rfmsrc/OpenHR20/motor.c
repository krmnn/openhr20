...
static volatile motor_dir_t MOTOR_Dir_Counter;  //!< actual direction for count pulses
    ...
    uint32_t MOTOR_counter;				//!< count volume of motor pulses for diagnostic
#endif
 ...
 *
 *	Parameter passed reflects current mount state of motor, and any key-related action:
 *  \param cal_type 0 = unmounted
 *  \param cal_type 1 = [do nothing?]
...
void MOTOR_updateCalibration(uint8_t cal_type)
{
	...
	else 
	{
		...
		else 
		{
			...
			{
                MOTOR_calibration_step++;		// Move to step 2
 ...
 * Note - in RS-485 mode, PE3 must only be changed while interrupts are disabled (to avoid conflict with RTS)
...
/*!
 *******************************************************************************
 * Pin change Interrupt INT0
 *
 * \note count light eye impulses: \ref MOTOR_PosAct
 *
...
{
    uint8_t pine=PINE;
    #if (!defined(COM_RECEIVE_ACTIVE) && ((defined COM_RS232) || (defined COM_RS485)))
        if ((pine & (1<<PE0)) == 0)		// RxD has gone low
		{
            RS_enable_rx();				// it is macro, not function
            PCMSK0 &= ~(1<<PCINT0);		// remove RxD from mask of monitored bits
        }
    #endif
    // motor eye - sensor pulses
        ...
        while (RFM_SDO_PIN & _BV(RFM_SDO_BITPOS)) {
          ...
          if (rfm_mode == rfmmode_tx) {
            ...
            if (rfm_framepos >= rfm_framesize) {
              ...
              task |= TASK_RFM; // inform the rfm task about end of transmission
              return; // \note !!WARNING!!
            }
...
ISR (TIMER0_OVF_vect)
{
    ...
    if ((uint8_t)(motor_diag_cnt>>8) >= config.motor_close_eye_timeout) 
	{
        ...
        // photo eye
        MOTOR_eye_disable();
        task&=~(TASK_MOTOR_PULSE);			// just insurance
        MOTOR_H_BRIDGE_stop();				// ensure that motor is stopped
    } 
	else 
	{
        if ( motor_timer > 0) 
